window.DocFlowPlugin = (() => {
  ...
  return {
        ...
        for (const row of rows) {
          ...
          if (row.invoiceDownloadable) {
            invoices.push({
              ...
              amount: row.amount,
              documentUrl: _marker(row.invoiceNumber, 'invoice', year),
          ...
          if (row.evnDownloadable) {
            invoices.push({
              ...
              date: row.date.toISOString(),
              amount: row.amount,
              documentUrl: _marker(row.invoiceNumber, 'evn', year),
    ...
    _internals: { _parseGermanDate, _parseAmount, _buildFilename, _marker, _parseMarker, _fmtDate },
  };
})();
